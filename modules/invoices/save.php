...
if($type==total_invoice && $saved) {
		...
		if (insertInvoiceItem($id,1,$product_id,$_POST["trading_type_id"],1,$_POST['tax_id'][0],$_POST['description'],$_POST['unit_price'])) {
			//$saved = true;
		}
		else {
...
elseif ($saved) {
		...
		while ($i <= $_POST['max_items']) {
			...
			{
				if (
				insertInvoiceItem($id,$_POST["quantity$i"],$_POST["products$i"],$_POST["trading_type_id"],$i,$_POST["tax_id"][$i],$_POST["description$i"], $_POST["unit_price$i"], $_POST["note_cost$i"] )
				) 
				{
				addItemUpdateCurrencyNote($_POST["trading_type_id"],$_POST["products$i"],$_POST["quantity$i"],$_POST["unit_price$i"]);
...
//	for($i=0;(!empty($_POST["quantity$i"]) && $i < $_POST['max_items']);$i++) {
		...
		if($_POST["delete$i"] !== "yes")
		{
			...
			if($_POST["quantity$i"] != null)
            {
				...
				if($_POST["line_item$i"] != "")
				{
					updateInvoiceItem($_POST["line_item$i"],$_POST["quantity$i"],$_POST["products$i"],$_POST["trading_type_id"],$i,$_POST['tax_id'][$i],$_POST["description$i"],$_POST["unit_price$i"],$_POST["note_cost$i"]);
					$saved;
					//$saved =  true;
/*
				}	
