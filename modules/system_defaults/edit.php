...
else if ($_GET["submit"] == "def_trading_type") {

	$defpay = getDefaultTradingType();
	$tradings = getActiveTradingTypes();
	

	if ($tradings == null) {
		//no records
		$display_block_trading_type = "<p><em>{$LANG['trading_type']}</em></p>";

	} else {
		$default = "trading_type";
		//has records, so display them
		$display_block_trading_type = <<<EOD
                <select name="value">
...
EOD;
		...
		foreach($tradings as $trading) {
			...
			$selected = $trading['id'] == $defaults['trading_type']?"selected style='font-weight: bold'":"";
			$escaped = htmlsafe($trading['description']);
			$display_block_trading_type .= <<<EOD
			<option $selected value="{$trading['id']}">
                        $escaped</option>
EOD;
		}
	}

	$description = "{$LANG['trading_type']}";
	$value = $display_block_trading_type;
