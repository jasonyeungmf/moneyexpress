...
function insertProductComplete($enabled=1,$visible=1,$description, 
	...
	if ($db_server == 'pgsql') {
		$sql = "INSERT into
			".TB_PREFIX."products
			(domain_id, country, symbol, currency_en, currency_cn, code, note_buy, note_sell, note_amount, note_cost, note_total, description,
			unit_price, custom_field1, custom_field2,custom_field3, custom_field4, notes, enabled, visible)
		VALUES
			(	
				:domain_id, :country, :symbol, :currency_en, :currency_cn, :code, :note_buy, :note_sell, :note_amount, :note_cost, :note_total,
				:description, :unit_price, :custom_field1, :custom_field2, :custom_field3, :custom_field4, :notes, :enabled, :visible
			)";
	} else {
		$sql = "INSERT into
			".TB_PREFIX."products
...
function insertAccount() {
	...
	$sql = "INSERT INTO 
			".TB_PREFIX."accounts
			(
				customer_no, name, payee, bank, account_no, domain_id, enabled
			)
			VALUES 
			(
				:customer_no, :name, :payee, :bank, :account_no, :domain_id, :enabled
...
function deleteAccount($module,$idField,$serial_no) {
	...
	// Tablename and column both pass whitelisting and FK checks
	$sql = "DELETE FROM ".TB_PREFIX."$module WHERE $s_idField = :serial_no";
    $logger->log("Item deleted: ".$sql, ZEND_Log::INFO);
	return dbQuery($sql, ':serial_no', $serial_no);
}
...
function insertCurrencyNoteComplete($enabled=1,$visible=1,$country, $symbol, $currency_en, $currency_cn, $code, $note_buy, $note_sell,
...
$sql = "SELECT code,note_buy,note_sell FROM ".TB_PREFIX."currencys_note order by code desc";
$sth = dbQuery($sql, ':domain_id',$auth_session->domain_id) or die(htmlsafe(end($dbh->errorInfo())));
...
for ($i = 4; $i <= 30; $i++) {
   ...
   if(in_array($code,$g_info)){
    ...
    $sql="UPDATE ".TB_PREFIX."currencys_note SET note_buy=$note_buy,note_sell=$note_sell WHERE code='$code'";
	$sth = dbQuery($sql, ':domain_id',$auth_session->domain_id) or die(htmlsafe(end($dbh->errorInfo())));
   }
}
}
...
function insertCurrencyTTComplete($enabled=1,$visible=1,$country, $symbol, $currency_en, $currency_cn, $code, $tt_buy, $tt_sell, $amount,
	...
	if ($db_server == 'pgsql') {
		$sql = "INSERT into
			...
			(domain_id, country, symbol, currency_en, currency_cn, code, tt_buy, tt_sell, amount, custom_field1, custom_field2, 
...
function insertCustomer() {
	...
	$sql = "INSERT INTO 
			".TB_PREFIX."customers
			(
				domain_id, id_copy, name, id_no, phone, fax, address, email, website, notes,
